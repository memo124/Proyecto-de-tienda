...
const API_MARCAS2U = '../../core/api/commerce/Marcas2U.php?action=';
...
function obtenerIdMarcasU(){
    $.ajax({
        dataType: 'json',
        url: API_MARCAS2U + 'obtenerID'
    })
    .done(function( response ) {
        ...
        if ( response.status ) {
            ...
            response.dataset.forEach(function( row ) {
                ...
                $('.CajaScrollMarcas2U').html( content );
                ...
                llenarCarruselesU(`${row.id_marcas}`);
            });
        }
    })
    .fail(function( jqXHR ) {
...
function llenarCarruselesU( id ){
    $.ajax({
        dataType: 'json',
        url: API_MARCAS2U + 'readProductos',
        data: { id_marcas: id },
        type: 'post'
    })
    .done(function( response ) {
