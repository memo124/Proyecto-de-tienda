...
require_once('../../models/commerce/Productos2.php');
...
if ( isset($_GET['action']) ) {
    ...
    switch ( $_GET['action'] ) {
        ...
        case 'obtenerID':
            if ( $result['dataset'] = $productos->readIdProductos() ) {
                $result['status'] = 1;
            } else {
        ...
        case 'readProductos':
            if ( $productos->setId($_POST['id_tipo']) ) {
                if ( $result['dataset'] = $productos->readProductos() ) {
                    $result['status'] = 1;
                } else {
