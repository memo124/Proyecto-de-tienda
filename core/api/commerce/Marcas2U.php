...
if ( isset($_GET['action']) ) {
    ...
    switch ( $_GET['action'] ) {
        ...
        case 'obtenerID':
            if ( $result['dataset'] = $marcasU->readIdMarcas() ) {
                $result['status'] = 1;
            } else {
        ...
        case 'readProductos':
            if ( $marcasU->setId($_POST['id_marcas']) ) {
                if ( $result['dataset'] = $marcasU->readProductos() ) {
                    $result['status'] = 1;
                } else {
